      ...
      <PageShell>
        ...
        <div className="sp-4col" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 14, marginBottom: 18 }}>
          {[
            ...
            { label: 'Tier', value: tierLabel(membership.loyaltyTier), detail: membership.commuterPassRoute?.label ?? 'No commuter pass selected yet', color: DS.blue },
          ].map((item) => (
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {[
                ...
                membership.dailyRoute
                  ? `Daily route memory is centered on ${membership.dailyRoute.label}, which is ${membership.dailyRoute.savingsPercent}% cheaper than solo movement when shared demand is healthy.`
              ...
              return (
                <div key={corridor.id} style={{ background: DS.card2, borderRadius: r(16), padding: '16px 16px 14px', border: `1px solid ${active ? `${DS.gold}35` : DS.border}` }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 8, marginTop: 12 }}>
                    {[
                      { label: 'Shared seat', value: `${corridor.sharedPriceJod} JOD` },
                      { label: 'Monthly pass', value: `${corridor.subscriptionPriceJod} JOD` },
                      { label: 'Savings', value: `${corridor.savingsPercent}%` },
                    ].map((item) => (
